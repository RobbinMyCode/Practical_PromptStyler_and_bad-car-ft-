...
def get_args():
    ...
    parser.add_argument("--prompts_file", default="",
                        help="the pickle-file in which the prompt (already encoded) are [required for weight init], if no file given, use 'a style of a [class]'")
    parser.add_argument("--word_mode", default="mean",
                        help="how the finetuning is computed a) mean [=default]: average all word vectors and finetune the representation b) linear: [=words are more important]: connect all word vectors with a linear layer and finetune those weights")
...
class Trainer:
    def __init__(self, args, device, tt, ww1, ww2, ww3, target_name):
        ...
        with open(path, 'rb') as handle:
            lin_style_weights = pickle.load(handle)

        self.lin_style_weights = torch.tensor(lin_style_weights, device=self.device, dtype=torch.float16,
        ...
        self.WEIGHTS = torch.nn.Parameter(
            torch.tensor(weight_total, requires_grad=True, device=self.device, dtype=torch.float16))
        self.car_ft_factor = torch.nn.Parameter(
            torch.tensor(0, requires_grad=True, device=self.device, dtype=torch.float16))

        if args.word_mode == "linear":
            self.linear_weight_cls = torch.nn.Parameter(
                1. / weight_total.shape[1] * torch.ones((weight_total.shape[1], 1), device=self.device,
                                                        dtype=torch.float16, requires_grad=True))
            self.linear_weight_ctx = torch.nn.Parameter(
                1. / weight_total.shape[0] * torch.ones((weight_total.shape[0], 1), device=self.device,
                                                        dtype=torch.float16, requires_grad=True))
    ...
    def _do_epoch(self):
        ...
        for it, ((data, class_l), d_idx) in enumerate(self.source_loader):
            ...
            p_ctx_stationary = softmax(
                (cos_sim(frozen_image_norm[:, None, :], self.weights_ctx[None, :, :])).type(torch.float32))
            cls_changing = cos_sim(CLIP_image_features_norm[:, None, :], self.weights_cls[None, :, :]).type(
                torch.float32)

            self.optimizer.zero_grad()
            predictions = frozen_image @ self.lin_style_weights + self.car_ft_factor * cls_changing
            print("Max/min difference:",torch.max(predictions - frozen_image @ self.lin_style_weights), torch.min(predictions - frozen_image @ self.lin_style_weights))
            # --- classification loss
            CrossEntropyLoss = CELoss(predictions, class_l)
            # --- kl loss
    ...
    def do_test(self, loader):
        ...
        for it, ((data, class_l), _) in enumerate(loader):
            ...
            cls_changing = cos_sim(CLIP_image_features_norm[:, None, :], self.weights_cls[None, :, :]).type(
                torch.float32)
            predictions = torch.nn.Softmax(dim=-1)(frozen_image @ self.lin_style_weights + self.car_ft_factor * cls_changing)
            predictions = torch.argmax(predictions, axis=-1)
            #predictions =
            # predictions = torch.argmax(nn.Softmax(dim=1).cuda()((CLIP_image_features @ self.weights_ctx.T).type(torch.float32)), dim=1)
            #print(predictions.size(), class_l.shape)
            class_correct += torch.sum(predictions == class_l).detach().cpu().numpy()
            total += len(class_l)
