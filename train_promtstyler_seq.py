...
class Trainer:
    def __init__(self, args, device, target_name):
        ...
        word_model_static = WordModel(self.clip_model, index_to_change=args.style_word_index+1,
    ...
    def _do_epoch(self):
        ...
        with torch.no_grad():
            total_output = self.word_model_static(class_words, style_index=self.n_style_vec)
    ...
    def do_training(self):
        ...
        print("init style words [:3]", self.word_model_static.style_words[:, :3])
        if train_noise:
            for self.n_style_vec in range(self.n_style_words):
                word_model_dynamic = WordModel(self.clip_model, index_to_change=self.args.style_word_index + 1,
                                              n_style_words=1, style_word_dim=self.text_feature_dim,
                                              word_basis=self.args.style_word_basis)
                ...
                #print("style words [:3]", self.word_model_static.style_words[:, :3])


        final_style_words = self.word_model_static.style_words.detach().clone()
