...
def get_args():
    parser = argparse.ArgumentParser(description="multi-basis PromptStyler, where StyleLoss can still change previous words")
    ...
    #these 2 are used if the targe tis not Terra
    ...
    parser.add_argument("--norm", type=int, default=0, help="0 dont norm, 1 norm, 2 use normed and not normed in linear layer")
...
class Trainer:
    ...
    def do_training(self):
        if self.args.dataset == "Terra":
            word_basises_start = ["a photo of a", "a nice photo of a", "a photo of the large", "a wildlife camera recording of a"]
            word_basises_end = ["", "", "", ""]
