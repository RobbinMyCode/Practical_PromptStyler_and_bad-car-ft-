...
class Trainer:
    def __init__(self, args, device):
        ...
        if args.norm:
            ...
            self.lin_projection_weights_i /= self.lin_projection_weights_i.norm(dim=-1, keepdim=True)
        ...
        rand_part = torch.nn.Parameter(torch.randn((self.feature_dim, self.feature_dim), requires_grad=False, device=self.device, dtype=torch.float16) * 0/self.feature_dim)
        eye_part = torch.nn.Parameter(torch.eye(self.feature_dim, requires_grad=False, device=self.device, dtype=torch.float16)*1)
        self.adapter_layer = rand_part + eye_part
        self.lin_projection_weights = torch.nn.Parameter(torch.tensor(self.lin_projection_weights_i, requires_grad=True, device=self.device, dtype=torch.float16))
        ...
        self.optimizer = torch.optim.SGD([self.lin_projection_weights], lr=1e-04, momentum=0.9)
    ...
    def _do_epoch(self):
        ...
        for it, (_, class_l, _, paths) in enumerate(self.source_loader):
            #if it > 1:#00: #shorter training on only first few
            #    break
            ...
            encodings = (CLIP_image_features @  self.adapter_layer @ self.lin_projection_weights.T)
            ...
            print((it + 1) * len(class_l), " / ", len(self.source_loader.dataset), ": ",
                  ...
                  "  Accuracy(total)=",
                  np.around(100 * n_corr / (it + 1) / len(class_l), 4), "%", "\t loss=", loss.detach().cpu().numpy(),
                  sep="", end="")
        ...
        for exponent in [self.exponent]:
            ...
            for it, (_, class_l, _, paths) in enumerate(loader):
                ...
                for iteration in range(len(CLIP_image_features) // self.n_splits):
                    ...
                    weighting_factor = (entropies[indices[0]] / entropies[indices, None]) ** exponent
