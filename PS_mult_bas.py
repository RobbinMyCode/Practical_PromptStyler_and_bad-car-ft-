...
def get_args():
    parser = argparse.ArgumentParser(description="Script to launch PromptStyler training with multiple StyleWord-Basises, "
    ...
    def do_training(self):
        ...
        for change_index, wb_start, wb_end in zip(pseudo_index, word_basises_start, word_basises_end):
            ...
            if first_run:
                ...
                #first_run = False # -- not savnig first basises but only using them as "pretraining" for first style is benetifitial (for best basis -> last basis)
            else:
